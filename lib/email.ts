...
type EmailContext = "waitlist" | "blog";

function buildBody(
  zip: string | null,
  recipientEmail?: string,
  context: EmailContext = "waitlist",
) {
  const isBlog = context === "blog";

  const subject = isBlog
    ? "You're subscribed to the Texergy AI blog"
    : "You're on the list — Texergy AI";

  const preheader = isBlog
    ? "You're subscribed to the Texergy AI blog."
    : "You're on the early-access waitlist for Texergy AI.";

  const section01Eyebrow = isBlog ? "Blog Subscription" : "01 / Early Access";
  const section01EyebrowHtml = isBlog ? "Blog&nbsp;Subscription" : "01&nbsp;/&nbsp;Early&nbsp;Access";
  const headlineHtml = isBlog
    ? "You're<br/>subscribed."
    : "You're on<br/>the list.";
  const headlineText = isBlog ? "YOU'RE SUBSCRIBED." : "YOU'RE ON THE LIST.";

  const introLine = isBlog
    ? "You're subscribed to the Texergy AI blog, our field notes on the Texas electricity market."
    : "You're confirmed for early access to Texergy AI, the smarter way for Texans to shop electricity plans.";

  const secondaryLine = isBlog
    ? "Plain-language writing about retail electricity, ERCOT, and how to stop overpaying for your bill. No marketing fluff."
    : zip
  ...
  const section02Eyebrow = isBlog ? "What to expect" : "02 / What's Next";
  const section02EyebrowHtml = isBlog ? "What&nbsp;to&nbsp;expect" : "02&nbsp;/&nbsp;What's&nbsp;Next";

  const bullets = isBlog
    ? [
        "A short note in your inbox whenever a new post goes up.",
        "Practical writing on plans, EFLs, and Texas grid quirks.",
        "Unsubscribe in one click. No marketing emails, ever.",
      ]
    : [
        "We'll email you the moment early access opens.",
        "You'll be among the first to try the AI-ranked plan matcher.",
        "100% free. No sign up required.",
      ];

  const footerLine = isBlog
    ? "You're getting this because you subscribed to the Texergy AI blog."
  ...
  const text = [
    "TEXERGYAI",
    section01Eyebrow.toUpperCase(),
    "",
    headlineText,
    "",
    introLine,
    "",
    secondaryLine.replace(/<[^>]+>/g, ""),
    "",
    section02Eyebrow.toUpperCase(),
    ...bullets.map((b) => `  → ${b}`),
    "",
          ...
          <tr><td style="padding:0 36px 32px;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
              ${bullets
                ...
                <td style="font-family:${mono};font-size:14px;line-height:1.7;color:#1a1a1a;padding-top:6px;">${escapeHtml(b)}</td>
              </tr>`,
                )
                .join("")}
            </table>
...
export async function sendNewsletterConfirmation(
  ...
  // One-click List-Unsubscribe: the URL form lets Gmail/Outlook fire a POST
  // straight to our unsubscribe endpoint without the user opening the email.
...
export async function sendWaitlistConfirmation(
  ...
  const { subject, text, html } = buildBody(zip, email, context);
