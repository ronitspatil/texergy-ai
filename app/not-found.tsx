  ...
  return (
    <main className="relative grid-bg flex min-h-[calc(var(--vph)*100)] flex-col items-center justify-center px-6 py-24 text-center">
      ...
      <div className="mt-10 flex flex-col items-center gap-3 sm:flex-row">
        <Link
          href="/"
          className="grain-surface rounded-full bg-accent shadow-e1 px-8 py-3 font-mono text-xs uppercase tracking-widest text-accent-foreground transition-all duration-200 hover:bg-accent-strong hover:shadow-e2"
        >
          Back to Home
        </Link>
        <Link
          href="/find/recommend"
          className="rounded-full border border-foreground/25 px-8 py-3 font-mono text-xs uppercase tracking-widest text-foreground/85 transition-colors duration-200 hover:border-accent hover:text-accent"
        >
          Find My Plan →
        </Link>
      </div>
      ...
      <p className="mt-12 font-mono text-[10px] uppercase tracking-[0.25em] text-muted-foreground/60">
        Texergy · Texas Electricity, Matched Smarter
      </p>
    </main>
  );
}
